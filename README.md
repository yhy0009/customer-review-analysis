...
[샘플 CSV](data/sample_reviews.csv) 40행 중 정제·분석을 마친 **34건**으로 실행한 실제 화면입니다(2026-09-23). 인사이트와 캡처는 `review-insights-v6`으로 다시 생성한 결과이며, 기존 감정 분류는 유지했습니다. [재생성 검증 기록](docs/INSIGHT_V6_VERIFICATION.md)에서 실행 조건과 확인 내용을 볼 수 있습니다.
...
이번 추출의 제품별 근거 주제 37개를 보존하며, 주제를 펼치면 리뷰 번호와 저장된 원문 인용을 확인할 수 있습니다. AI를 다시 호출하면 주제의 표현·묶음과 개수는 달라질 수 있습니다.
...
검증 범위는 입력·정제·중복 처리·선택 항목 누락, SQLite 트랜잭션, AI 응답 검증과 실패 처리, 추출 근거·저장 결과 무효화, CLI 연결, 산출물·필터·웹 동작입니다. 아래는 **2026-09-23, 기능 커밋 `ed205cf` 기준** 검증 기록입니다.

| 검증 | 결과 |
|---|---|
| Python 전체 테스트 | 554개 통과 |
| JavaScript 테스트 | 13개 통과 |
| 기본 AI 평가셋 형식 | 합성 리뷰 18건 검증 통과, 실제 API 호출 없음 |
| 샘플 수집·정제 | 40행 입력 → 원본 38건 → 정제 34건, 평균 별점 3.00 |
| v6 실제 인사이트 재생성 | 분석 완료 리뷰 34건, 근거 추출 2회·요약 1회 성공, 내부 필드명 노출 없음 |
| 웹·단일 HTML 실제 데이터 확인 | 동일한 v6 요약·근거 37개 표시, 근거 펼치기와 원문 인용 확인, README 캡처 4장 갱신 |
