...
일반·분할 요약 모두 장점 label의 **문자열 값만** 자연스러운 한국어 문장에 포함하도록
지시한다. 요약·이슈·개선 제안 문자열에 `praise_label`, `issue_candidates` 같은 내부
필드명이나 `label: 값` 같은 메타데이터가 노출되면 형식 오류로 거부한다. 대소문자,
전각 문자, Markdown의 이스케이프 표기도 검사하며, 원문 인용을 임의로 고치지 않는다.
...
프롬프트 버전은 `review-insights-v6`이며, `AnalysisOptions.prompt_version`은
`None` 또는 이 버전을 사용한다. 감정 분석 전용 버전은 적용할 수 없다.
이전 인사이트 버전을 명시했던 호출자는 v6 또는 `None`으로 변경해야 한다. 기존 개별 감정
분석 결과와 DB 스키마는 변경하지 않는다. 이전 프롬프트는 `evaluation/prompts/`에 보존한다.
기존 v5 저장본은 현재 설정의 캐시로 재사용하지 않으므로 `extract`를 다시 실행하고
`dashboard --html`로 리포트를 다시 만든다. 웹에 파일을 명시했다면 새 저장본을 연결한다.

## 대량 입력과 근거 보존

v5부터 선택한 리뷰를 ID 순으로 최대 20건씩 분할한다. 각 근거 요청의 사용자 JSON은
