...
Raw 리뷰만 있는 경우 대상에 포함되지 않는다. `import`, `clean`, `dashboard`의 기본 CLI
연결은 후속 작업이며 현재 데이터 적재에는 기존 수집·정제 모듈을 사용한다.
...
추출 대상이 0건이면 안내만 출력하고 API를 호출하지 않는다. 입력 JSON이 기본
24,000자를 넘으면 해당 단계 API 호출 전에 오류를 반환하므로 필터나 `--limit`으로 대상을 줄인다.
현재 추출은 리뷰별 근거 확인과 최종 요약의 두 단계로 요청한다. 두 단계 모두 DB 읽기
