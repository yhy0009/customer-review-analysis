...
이 명령은 추가 AI 호출 없이 저장된 통계와 현재 데이터에 일치하는 `extract` 결과를 함께 사용한다.
...
터미널 제어 문자를 제거하며, 성공 결과를 `paths.output_dir/insights/` 아래 JSON으로 저장하고
절대 경로를 출력한다. `dashboard`의 저장 결과 검증·포함 규칙은 [대시보드 CLI 안내](DASHBOARD.md)를 따른다.
