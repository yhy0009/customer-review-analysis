...
- `stats`: 정제 리뷰 수, 분석 완료·미분석·실패 수, 분석 완료율, 평균 별점과 감정 분포를 표시한다.
  ...
  미분석 수는 실패 수를 제외한다. 평균 별점은 별점이 있는 정제 리뷰만 사용한다.
  비율과 평균의 반올림은 출력 단계에서만 수행한다.
- 제품명·작성일·별점 누락은 `제품명 없음`·`날짜 없음`·`N/A`로 표시한다.
  지정된 필터의 값이 없는 리뷰는 제외되며 날짜·별점 정렬의 누락값은 양 방향 모두 마지막이다.
