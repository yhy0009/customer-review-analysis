...
- 공개 작업 상태에 선택 필드 `error_code`, `retry_action`을 추가한다. 실패 시 허용된
  오류 코드로 서버가 안내문을 만들며 예외 원문·자격 증명은 전달하지 않는다. 진행·성공은
  두 필드가 null이다. `retry_action`은 `retry`, `check_settings`, `check_storage`,
  `change_scope`이며 UI의 안내만 바꾼다. 자동 재시도나 추가 AI 호출을 발생시키지 않는다.
...
### 25.3 웹 조회 조건과 브라우저 탐색

- JS 화면에 정확한 별점/최소 별점 선택을 추가한다. 기존 `rating`/`rating_min` 계약을
  사용하며 두 조건을 함께 보내지 않는다. 통계·리뷰·차트·인사이트는 동일 필터를 사용한다.
- 적용한 필터와 리뷰 페이지는 URL 쿼리로, 화면 탭은 해시로 복원한다. 조건 변경은
  1페이지로 이동하고 뒤로가기·앞으로가기·새로고침은 해당 주소의 조건을 다시 조회한다.
- 잘못된 알려진 조건은 화면 오류로 처리하고, 알 수 없는 URL 매개변수는 무시한다.
  데이터 감소로 페이지가 사라지면 유효한 마지막 페이지로 주소와 화면을 함께 보정한다.
- URL 탐색은 조회 요청만 수행한다. 기존 HTTP snapshot v1, 공통 DTO, 저장소와 AI 생성
  계약은 변경하지 않는다.

### 25.4 조회 시점의 리뷰 파일 다운로드

- snapshot v1의 선택 필드 `export`는 status/row_count/limit/urls를 제공한다.
  `/api/export/{snapshot_id}/csv|jsonl`은 현재 필터 전체를 ID 오름차순으로 내보낸다.
- 통계·페이지와 동일한 읽기 트랜잭션에서 최대 2000건을 보관한다. 한도를 초과하면
  `too_large`와 빈 URL 목록을 반환하며 다운로드 요청은 413이다. 부분 내보내기는 하지 않는다.
- 다운로드는 보관된 데이터와 기존 `FileReviewExporter`를 사용한다. 외부 리뷰 ID는 빈
