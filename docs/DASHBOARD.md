...
- 유효한 추출 결과가 있으면 AI 요약·이슈·개선 제안·키워드·원문 근거도 함께 내장한다.
  웹 대시보드처럼 큰 요약 카드, 주요 이슈·개선 제안의 2열 목록, 제품별 원문 근거로 구성한다.
  좁은 화면에서는 1열로 표시한다. 근거 주제를 클릭하거나 키보드로 펼치면 리뷰 번호와
  저장된 인용문을 볼 수 있다. 모든 주제와 인용을 포함하며, 건수는 주제별 중복 없는 리뷰 수다.
  단일 파일에는 인용문을 저장하며, 웹 화면의 전체 리뷰 조회 기능은 포함하지 않는다.
  인사이트의 필터·실제 추출 건수·생성 시각과 별도의 키워드 집계도 표시한다.
...
필터·생성 시각·선택적 알림과 인사이트를 받아 HTML 문자열을 반환한다.
`src/html_insights.py`가 인사이트 카드와 HTML 기본 `details` 요소의 근거 목록을 구성한다.
파일 저장과 산출물 경로 검증은
